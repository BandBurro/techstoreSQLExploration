...
DROP FUNCTION IF EXISTS fn_audit_product_price_change();
DROP FUNCTION IF EXISTS fn_get_customer_total_spent(INTEGER);
DROP PROCEDURE IF EXISTS sp_process_new_order(INTEGER, INTEGER, INTEGER);
...
CREATE INDEX idx_price_audit_product ON price_audit(product_id);
CREATE INDEX idx_price_audit_date ON price_audit(changed_at);
...
BEGIN
    ...
    -- Commit transaction (implicit in procedures, but explicit for clarity)
    COMMIT;
...
-- Create role for group members (adjust role name as needed)
-- Note: Replace 'group_member_1', 'group_member_2' with actual usernames
DO $$
BEGIN
    ...
    GRANT EXECUTE ON PROCEDURE sp_process_new_order TO techstore_developer;
    ...
    ALTER DEFAULT PRIVILEGES IN SCHEMA db_techstore 
        GRANT EXECUTE ON FUNCTIONS TO techstore_developer;
    -- Note: ALTER DEFAULT PRIVILEGES doesn't support PROCEDURE keyword
    -- Future procedures will need explicit GRANT EXECUTE ON PROCEDURE statements
END $$;
...
BEGIN
    ...
    GRANT EXECUTE ON PROCEDURE sp_process_new_order TO gerente;
    ...
    ALTER DEFAULT PRIVILEGES IN SCHEMA db_techstore 
        GRANT EXECUTE ON FUNCTIONS TO gerente;
    -- Note: ALTER DEFAULT PRIVILEGES doesn't support PROCEDURE keyword
    -- Future procedures will need explicit GRANT EXECUTE ON PROCEDURE statements
END $$;
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'gerente_1') THEN
        CREATE ROLE gerente_1 LOGIN PASSWORD '123456';
    END IF;

    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = 'desenvolvedor_1') THEN
        CREATE ROLE desenvolvedor_1 LOGIN PASSWORD '1234567';
    END IF;
END $$;
-- This "INHERIT" behavior is what gives the users the permissions defined above
